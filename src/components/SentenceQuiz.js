...
const SentenceQuiz = () => {
  ...
  const beginQuiz = () => {
    ...
    for (let i = 0; i < Math.min(10, sentencesList.length); i++) {
      let randomIndex;
      do {
      ...
      const sentence = sentencesList[randomIndex];
      
      const wrongAnswers = [];
      while (wrongAnswers.length < 3) {
        const wrongIndex = Math.floor(Math.random() * sentencesList.length);
        const wrongSentence = sentencesList[wrongIndex];
      ...
      const options = shuffleArray([sentence.answer, ...wrongAnswers]);
  ...
  return (
    <div id="sentence-quiz" className="tab-content active">
      <h2>문장퀴즈</h2>
